...
puts coffee_drink?(%w[tea coffee water espresso])
...
puts correct_guess?([3, 2, 15, 11, 10, 15], 15)
...
puts twenty_first_century_years?([2002, 2099, 2086, 2049, 2020])

def correct_format?(word_list)
  # use #none? to return true when none of the words in the word_list (array) are in upcase
  word_list.none?{ |el| !el.upcase }
end

puts correct_format?(%w[pen pencil pages])
...
puts valid_scores?({Lawrence: 4, Taylor: 2, Arridge: 5}, 5)
